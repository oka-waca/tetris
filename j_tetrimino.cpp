...
JTetrimino::JTetrimino(PlayArea* p_play_area) : Tetrimino(p_play_area){
  absolute_coord.x = 3;
  absolute_coord.y = 19;
  facing_ = 'N';
  
  for(auto rel_coord:mino_relative_coord_N_){
    minos_.push_back(Mino('J', absolute_coord.x + rel_coord.x, absolute_coord.y + rel_coord.y));
  }
}
...
bool JTetrimino::DoSuperRotationClockwise(){
  bool tmp = false;
  if(facing_ == 'N'){
    for(int i=0; i<5; i++){
      tmp_minos_.clear();
      ...
      for(auto coord:mino_relative_coord_E_){
        tmp_minos_.push_back(Mino('J', coord.x+tmp_abs_x, coord.y+tmp_abs_y));
      }
...
bool JTetrimino::DoSuperRotationCounterClockwise(){
  bool tmp = false;
  if(facing_ == 'N'){
    for(int i=0; i<5; i++){
      tmp_minos_.clear();
