...
ITetrimino::ITetrimino(PlayArea* p_play_area) : Tetrimino(p_play_area){
  ...
  for(auto rel_coord:mino_relative_coord_N_){
    minos_.push_back(Mino('I', absolute_coord.x + rel_coord.x, absolute_coord.y + rel_coord.y));
  }
...
bool ITetrimino::DoSuperRotationClockwise(){
  bool tmp = false;
  if(facing_ == 'N'){
    for(int i=0; i<5; i++){
      tmp_minos_.clear();
      ...
      for(auto coord:mino_relative_coord_E_){
        tmp_minos_.push_back(Mino('I', coord.x+tmp_abs_x, coord.y+tmp_abs_y));
      }
...
bool ITetrimino::DoSuperRotationCounterClockwise(){
  bool tmp = false;
  if(facing_ == 'N'){
    for(int i=0; i<5; i++){
      tmp_minos_.clear();
